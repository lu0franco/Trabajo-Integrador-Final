 ...
 * @param[in] length  Length of input report data buffer
 */
// Estructura idéntica al parser de Arduino
typedef struct __attribute__((packed)) {
    uint8_t  rptid;
    uint8_t  xl, xh;
    uint8_t  yl, yh;
    uint8_t  zl, zh;
} SpaceNavigatorEventData;
                                             ...
                                             const int length)
{
    ...
    case 1:                       // -------- TRANSLATION --------
        if (length >= 7) {
            tx = (int16_t)(data[1] | (data[2] << 8));
            ty = (int16_t)(data[3] | (data[4] << 8));
            tz = (int16_t)(data[5] | (data[6] << 8));
            printf("T: %+6d %+6d %+6d", tx, ty, tz);

            if (length == 13) {   // rotación incluida
                rx = (int16_t)(data[7]  | (data[8]  << 8));
                ry = (int16_t)(data[9]  | (data[10] << 8));
                rz = (int16_t)(data[11] | (data[12] << 8));
                printf("   R: %+6d %+6d %+6d", rx, ry, rz);
            }
            printf("\n");
        }
        break;

    case 2:                       // -------- ROTATION (solo) --------
        if (length >= 7) {
            rx = (int16_t)(data[1] | (data[2] << 8));
            ry = (int16_t)(data[3] | (data[4] << 8));
            rz = (int16_t)(data[5] | (data[6] << 8));
            printf("R: %+6d %+6d %+6d\n", rx, ry, rz);
        }
        break;
